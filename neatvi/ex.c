...
int xshape = 0;//1			/* perform letter shaping */
...
static int ex_modifiedbuffer(char * c,char *msg)
{
    if (!lbuf_modified(xb))
        return 0;
    if (xaw && ex_path()[0])
...
static int ec_quit(char *ec)
{
    ...
    ex_cmd(ec, cmd);
    if (!strchr(cmd, '!'))
        if (ex_modifiedbuffer(ec,"buffer modified\n"))
            return 1;
    xquit = 1;
    return 0;
}
...
static int ec_edit(char *ec)
{
    ...
    ex_arg(ec, arg);
    if (!strchr(cmd, '!'))
        if (xb && ex_modifiedbuffer(ec,"buffer modified\n"))
            return 1;
...
static int ec_glob(char *ec)
{
    ...
    while (i < lbuf_len(xb)) {
        char *ln = lbuf_get(xb, i);
        if ((rset_find(re, ln, LEN(offs) / 2, offs, 0) < 0) == not1) {
            xrow = i;
            if (ex_exec(s))
                break;
            i = MIN(i, xrow);
